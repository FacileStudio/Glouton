<script lang="ts">
  ...
  // Everything that changes and updates the UI must be wrapped in $state()
  ...
  let statusFilter = $state<'HOT' | 'WARM' | 'COLD' | undefined>(undefined);
  let countryFilter = $state('');
  let cityFilter = $state('');
  let businessTypeFilter = $state<'all' | 'domain' | 'local'>('all');
  let contactedFilter = $state<'all' | 'contacted' | 'not_contacted'>('all');
  ...
  let deletingAuditId = $state<string | null>(null);
  
  let sortBy = $state<'createdAt' | 'score' | 'domain' | 'status'>('createdAt');
  ...
  async function loadData() {
    ...
    try {
      ...
      // Safely assign data, ensuring we always have arrays to prevent .find/.filter crashes
      leads = leadsData?.leads || [];
      totalPages = leadsData?.pagination?.totalPages || 1;
      totalLeads = leadsData?.pagination?.total || 0;
      
      auditSessions = Array.isArray(auditSessionsData) ? auditSessionsData : [];
  ...
  $effect(() => {
    // Explicitly read variables so the effect knows to track them
    searchFilter; countryFilter; cityFilter; statusFilter; contactedFilter; businessTypeFilter;
  ...
  function calculateETA(session: AuditSession, currentTime?: Date): string {
    if (!session.startedAt || session.progress >= 100 || !currentTime) return 'Calculating...';
    if (session.progress === 0) return 'Starting...';
  ...
  let filteredLeads = $derived.by(() => {
    // Safety check included: ensure `leads` is an array
  ...
  let localBusinessLeads = $state([]);
  let hasLocalBusinesses = $derived(localBusinessLeads.length > 0);

  // Bulletproof derived check
  let activeAudit = $derived(
    Array.isArray(auditSessions) 
...
<div
  class="p-6 lg:p-12 max-w-[1600px] mx-auto space-y-12 font-sans"
  style="selection-background-color: #FEC129; selection-color: black;"
>
    ...
    <div class="bg-gradient-to-r from-blue-500 to-purple-600 rounded-[32px] p-8 shadow-xl" in:fade>
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div class="flex items-center gap-4">
          ...
          >
            <iconify-icon icon="solar:shield-check-bold" width="32" class="text-white"
            ></iconify-icon>
          </div>
          <div class="space-y-2">
            ...
            <p class="text-white/90 font-medium">
              Processing {activeAudit.processedLeads.toLocaleString()} / {activeAudit.totalLeads.toLocaleString()}
              leads
          ...
          {#if activeAudit.startedAt}
            <div class="text-right">
              ...
              <p class="text-white text-xl font-black">
                {formatElapsedTime(activeAudit.startedAt, now)}
              </p>
            </div>
            <div class="text-right">
      ...
      <div class="mt-6">
        <div class="flex items-center justify-between mb-2">
          <span class="text-white/70 text-sm font-bold"
            >{activeAudit.progress.toFixed(1)}% Complete</span
          >
          <span class="text-white/70 text-sm font-bold"
            >{activeAudit.processedLeads} / {activeAudit.totalLeads}</span
          >
        </div>
  ...
  <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
    {#if initialLoading}
      ...
      {/each}
    {:else}
      {#each [{ label: 'Total Leads', val: stats?.totalLeads?.toLocaleString() || '0', icon: 'solar:box-bold', color: 'text-black' }, { label: 'Hot Leads', val: stats?.hotLeads?.toString() || '0', icon: 'solar:fire-bold', color: 'text-orange-500' }, { label: 'Warm Leads', val: stats?.warmLeads?.toString() || '0', icon: 'solar:star-bold', color: 'text-yellow-500' }, { label: 'Total Emails', val: stats?.totalEmails?.toLocaleString() || '0', icon: 'solar:letter-bold', color: 'text-blue-500' }, { label: 'Total Phones', val: stats?.totalPhoneNumbers?.toLocaleString() || '0', icon: 'solar:phone-bold', color: 'text-green-500' }] as stat}
        <div
  ...
  <section style="background-color: #EFEAE6;" class="rounded-[40px] overflow-hidden shadow-lg">
        ...
        {#if businessTypeFilter === 'local' || hasLocalBusinesses}
          <div class="flex bg-neutral-100 rounded-xl p-1 border border-neutral-200">
            <button
              ...
              class="px-4 py-2 rounded-lg text-xs font-black uppercase transition-all {viewMode ===
              'list'
                ? 'bg-white shadow-sm'
      ...
      {:else if businessTypeFilter === 'local' && viewMode === 'list'}
        {#if localBusinessLeads.length === 0}
          <div class="flex flex-col items-center justify-center py-20 space-y-4">
            <iconify-icon icon="solar:ghost-bold" width="80" class="text-neutral-200"
            ></iconify-icon>
            ...
            <tr class="bg-white border-b-2 border-neutral-200">
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  {#if sortBy === 'domain'}
                    <iconify-icon
                      icon="solar:{sortOrder === 'asc' ? 'alt-arrow-up' : 'alt-arrow-down'}-bold"
                      width="14"
                    ></iconify-icon>
                  {/if}
                </div>
              ...
              <th
                class="px-4 py-4 text-xs font-bold uppercase tracking-wide text-neutral-500 w-[17%]"
                >Contact</th
              >
              <th
                class="px-4 py-4 text-xs font-bold uppercase tracking-wide text-neutral-500 w-[8%]"
                >City</th
              >
              <th
                class="px-4 py-4 text-xs font-bold uppercase tracking-wide text-neutral-500 w-[8%]"
              ...
              >
              <th
                class="px-4 py-4 text-xs font-bold uppercase tracking-wide text-neutral-500 w-[11%]"
                >Status</th
              >
              <th
              ...
              <th
                class="px-4 py-4 text-xs font-bold uppercase tracking-wide text-neutral-500 w-[15%]"
                >Technologies</th
              >
              <th
                class="px-4 py-4 text-xs font-bold uppercase tracking-wide text-neutral-500 cursor-pointer hover:bg-neutral-50 transition-colors select-none w-[9%]"
              ...
              >
                <td class="px-4 py-4">
                  <div class="flex items-center gap-2 min-w-0">
                    <div
                      class="w-10 h-10 rounded-xl bg-white flex items-center justify-center overflow-hidden flex-shrink-0 shadow-md group-hover:shadow-lg transition-shadow"
                    >
                      {#if lead.domain}
                        <img
                        ...
                        />
                      {:else}
                        <iconify-icon icon="solar:global-bold" width="20" class="text-neutral-300"
                        ></iconify-icon>
                      {/if}
                    </div>
                    <div class="flex flex-col min-w-0 flex-1">
                      <span
                        class="text-neutral-900 font-bold text-sm group-hover:text-[#FEC129] transition-colors truncate"
                        title={lead.domain || 'No domain'}>{lead.domain || 'No domain'}</span
                      >
                    </div>
                  </div>
                </td>
                <td class="px-4 py-4">
                  <div class="flex flex-col min-w-0">
                    {#if lead.email}
                      <span class="text-neutral-700 font-medium text-xs truncate" title={lead.email}
                        >{lead.email}</span
                      >
                    {:else if lead.firstName || lead.lastName}
                      <span
                        class="text-neutral-700 font-medium text-xs truncate"
                ...
                <td class="px-4 py-4">
                  <span
                    class="text-neutral-700 font-medium text-xs truncate block"
                    title={lead.city || '-'}>{lead.city || '-'}</span
                  >
                </td>
                <td class="px-4 py-4">
                  <span
                    class="text-neutral-700 font-medium text-xs truncate block"
                    title={lead.country || '-'}>{lead.country || '-'}</span
                  >
                </td>
                <td class="px-4 py-4">
                  {#if lead.contacted}
                    <div class="flex items-center gap-1.5 min-w-0">
                      ...
                      {#if lead.emailsSentCount > 0}
                        <span
                          class="px-1.5 py-0.5 bg-green-100 text-green-700 rounded text-[10px] font-bold flex-shrink-0"
                        >
                ...
                <td class="px-4 py-4">
                  <span
                    class="px-2 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wide inline-block
                                  {lead.status === 'HOT'
                  ...
                  <div class="flex flex-wrap gap-1.5 min-w-0">
                    {#if lead.technologies.length > 0}
                      {#each lead.technologies.slice(0, 2) as tech}
                        <span
                          class="px-2 py-1 bg-neutral-100 group-hover:bg-neutral-200 rounded-md text-[10px] font-semibold text-neutral-600 transition-colors truncate max-w-[60px]"
                          title={tech}>{tech}</span
                        >
                      {/each}
                      {#if lead.technologies.length > 2}
                        <span
                          class="px-2 py-1 bg-neutral-200 group-hover:bg-neutral-300 rounded-md text-[10px] font-bold text-neutral-700 transition-colors flex-shrink-0"
                          title={lead.technologies.slice(2).join(', ')}
                        >
                ...
                <td class="px-4 py-4">
                  <div class="flex items-center gap-2 min-w-0">
                    <div class="w-12 h-2 bg-neutral-200 rounded-full overflow-hidden flex-shrink-0">
                      <div
                        class="h-full transition-all {lead.score >= 70
                          ? 'bg-green-500'
                          : lead.score >= 40
                            ? 'bg-yellow-500'
                            : 'bg-orange-500'}"
                    ...
                    <span class="text-xs font-bold text-neutral-600 flex-shrink-0"
                      >{lead.score}</span
                    >
                  </div>
                </td>
                <td class="px-4 py-4">
                  <span class="text-xs font-medium text-neutral-500"
                    >{formatTimeAgo(lead.createdAt)}</span
                  >
                </td>
              </tr>
            {/each}
    ...
    <div
      class="px-10 py-6 border-t border-neutral-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-neutral-50/50"
    >
      <div class="flex items-center gap-4">
        <select
          bind:value={pageSize}
