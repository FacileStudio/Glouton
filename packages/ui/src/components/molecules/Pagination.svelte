...
<nav class={cn('flex items-center justify-center gap-1', className)} aria-label="Pagination">
  <button
    ...
    class="flex h-9 w-9 items-center justify-center rounded-xl text-neutral-500 transition-all hover:bg-neutral-100 hover:text-black disabled:opacity-30 disabled:hover:bg-transparent cursor-pointer disabled:cursor-not-allowed"
  >
    ...
    {#each pages as pageNum, idx (idx)}
      ...
      {:else}
        <button
          ...
          class={cn(
            'h-9 min-w-[36px] px-2 rounded-xl text-sm font-bold transition-all duration-200 cursor-pointer',
            currentPage === pageNum
